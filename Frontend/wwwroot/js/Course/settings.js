...
document.getElementById("settingsForm").addEventListener("submit", async function (e) {
    ...
    try {
        await apiClient.put(`/api/Course/${classId}/settings`, {
            className: className,
...
async function inviteStudentByEmail() {
    ...
    try {
        const response = await apiClient.post(`/api/Course/${classId}/invite`, { email: email });
...
async function toggleClassStatus() {
    ...
    showConfirm(`Bạn có chắc muốn ${action} lớp học này không?`, "Xác nhận", async () => {
        ...
        try {
            const data = await apiClient.post(`/api/Course/${classId}/${endpoint}`);
