...
async function loadSettings() {
    ...
    try {
        ...
        currentInviteLink = `${baseUrl}/Course/Join?code=${data.invitationCode}`;
...
async function inviteStudentByEmail() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/Course/${classId}/invite`, {
            method: 'POST',
            headers: { 
                "Authorization": "Bearer " + token,
                "Content-Type": "application/json"
        ...
        if (response.ok) {
            ...
            const err = await response.text();
            showToast(err || "Lỗi khi gửi lời mời", "error");
        }
    } catch (error) {
        showToast("Đã xảy ra lỗi mạng.", "error");
    } finally {
...
async function toggleClassStatus() {
    ...
    showConfirm(`Bạn có chắc muốn ${action} lớp học này không?`, "Xác nhận", async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/Course/${classId}/${endpoint}`, {
                method: "POST",
                headers: { "Authorization": "Bearer " + token }
            });
            ...
            if (res.ok) {
                ...
                showToast(err?.message || `Lỗi khi ${action} lớp học.`, "error");
            }
        } catch (error) {
            console.error(error);
